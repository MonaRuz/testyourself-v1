  ...
  return (
    <div>
        ...
        {categories.map((category)=>{
          return<li onClick={()=>navigate(`/category/overview/${category.id}`)} className="border border-yellow-200 hover:border hover:border-none hover:bg-yellow-200 hover:text-black my-1 py-2" key={category.id}>{category.category}</li>
        })}
        </ul>
    </div>
  )
